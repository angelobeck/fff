...
class SectionAccessibilityReview_Team_App extends ApplicationHelper {

    static isChild(parent, name) {
        var data = store.domainContent.openByName(name);
        if (data.parentName === parent.parent.name && data.type === "team") {
            return true;
        } else {
            return false;
        }
...
return store.domainContent.childrenNames(parent.parent.name, "team");
    }
